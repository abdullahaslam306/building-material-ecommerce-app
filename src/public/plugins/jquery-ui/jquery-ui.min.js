...
(function (e) { typeof define === 'function' && define.amd ? define(['jquery'], e) : e(jQuery); }((e) => {
  function t(t, s) { let n; let a; let o; const r = t.nodeName.toLowerCase(); return r === 'area' ? (n = t.parentNode, a = n.name, t.href && a && n.nodeName.toLowerCase() === 'map' ? (o = e(`img[usemap='#${a}']`)[0], !!o && i(o)) : !1) : (/^(input|select|textarea|button|object)$/.test(r) ? !t.disabled : r === 'a' ? t.href || s : s) && i(t); } function i(t) { return e.expr.filters.visible(t) && !e(t).parents().addBack().filter(function () { return e.css(this, 'visibility') === 'hidden'; }).length; } function s(e) { for (var t, i; e.length && e[0] !== document;) { if (t = e.css('position'), (t === 'absolute' || t === 'relative' || t === 'fixed') && (i = parseInt(e.css('zIndex'), 10), !isNaN(i) && i !== 0)) return i; e = e.parent(); } return 0; } function n() {
    ...
    _processPanels() { const e = this.headers; const t = this.panels; this.headers = this.element.find(this.options.header).addClass('ui-accordion-header ui-state-default ui-corner-all'), this.panels = this.headers.next().addClass('ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom').filter(':not(.ui-accordion-content-active)').hide(), t && (this._off(e.not(this.headers)), this._off(t.not(this.panels))); },
    _refresh() {
    ...
    _create() {
      this.activeMenu = this.element, this.mouseHandled = !1, this.element.uniqueId().addClass('ui-menu ui-widget ui-widget-content').toggleClass('ui-menu-icons', !!this.element.find('.ui-icon').length).attr({ role: this.options.role, tabIndex: 0 }), this.options.disabled && this.element.addClass('ui-state-disabled').attr('aria-disabled', 'true'), this._on({
        ...
        focus(e, t) { const i = this.active || this.element.find(this.options.items).eq(0); t || this.focus(e, i); },
        blur(t) { this._delay(function () { e.contains(this.element[0], this.document[0].activeElement) || this.collapseAll(t); }); },
        keydown: '_keydown',
    ...
    _doKeyPress(t) {
      let i; let s; const n = e.datepicker._getInst(t.target);
  ...
  }), e.fn.datepicker = function (t) { if (!this.length) return this; e.datepicker.initialized || (e(document).mousedown(e.datepicker._checkExternalClick), e.datepicker.initialized = !0), e(`#${e.datepicker._mainDivId}`).length === 0 && e('body').append(e.datepicker.dpDiv); const i = Array.prototype.slice.call(arguments, 1); return typeof t !== 'string' || t !== 'isDisabled' && t !== 'getDate' && t !== 'widget' ? t === 'option' && arguments.length === 2 && typeof arguments[1] === 'string' ? e.datepicker[`_${t}Datepicker`].apply(e.datepicker, [this[0]].concat(i)) : this.each(function () { typeof t === 'string' ? e.datepicker[`_${t}Datepicker`].apply(e.datepicker, [this].concat(i)) : e.datepicker._attachDatepicker(this, t); }) : e.datepicker[`_${t}Datepicker`].apply(e.datepicker, [this[0]].concat(i)); }, e.datepicker = new n(), e.datepicker.initialized = !1, e.datepicker.uuid = (new Date()).getTime(), e.datepicker.version = '1.11.4', e.datepicker, e.widget('ui.draggable', e.ui.mouse, {
    version: '1.11.4',
    ...
    _hasScroll(t, i) { if (e(t).css('overflow') === 'hidden') return !1; const s = i && i === 'left' ? 'scrollLeft' : 'scrollTop'; let n = !1; return t[s] > 0 ? !0 : (t[s] = 1, n = t[s] > 0, t[s] = 0, n); },
    _create() {
    ...
    open() { const t = this; return this._isOpen ? (this._moveToTop() && this._focusTabbable(), void 0) : (this._isOpen = !0, this.opener = e(this.document[0].activeElement), this._size(), this._position(), this._createOverlay(), this._moveToTop(null, !0), this.overlay && this.overlay.css('z-index', this.uiDialog.css('z-index') - 1), this._show(this.uiDialog, this.options.show, () => { t._focusTabbable(), t._trigger('focus'); }), this._makeFocusTarget(), this._trigger('open'), void 0); },
    ...
    _createWrapper() {
      this.uiDialog = e('<div>').addClass(`ui-dialog ui-widget ui-widget-content ui-corner-all ui-front ${this.options.dialogClass}`).hide().attr({ tabIndex: -1, role: 'dialog' })
        ...
        keydown(t) {
          ...
          if (t.keyCode === e.ui.keyCode.TAB && !t.isDefaultPrevented()) { const i = this.uiDialog.find(':tabbable'); const s = i.filter(':first'); const n = i.filter(':last'); t.target !== n[0] && t.target !== this.uiDialog[0] || t.shiftKey ? t.target !== s[0] && t.target !== this.uiDialog[0] || !t.shiftKey || (this._delay(() => { n.focus(); }), t.preventDefault()) : (this._delay(() => { s.focus(); }), t.preventDefault()); }
        },
        mousedown(e) { this._moveToTop(e) && this._focusTabbable(); },
  ...
  }, e.ui.droppable; const y = 'ui-effects-'; const b = e; e.effects = { effect: {} }, (function (e, t) {
    ...
    function t(t, i, s, n) { return e.isPlainObject(t) && (i = t, t = t.effect), t = { effect: t }, i == null && (i = {}), e.isFunction(i) && (n = i, s = null, i = {}), (typeof i === 'number' || e.fx.speeds[i]) && (n = s, s = i, i = {}), e.isFunction(s) && (n = s, s = null), i && e.extend(t, i), s = s || i.duration, t.duration = e.fx.off ? 0 : typeof s === 'number' ? s : s in e.fx.speeds ? e.fx.speeds[s] : e.fx.speeds._default, t.complete = n || i.complete, t; } function i(t) { return !t || typeof t === 'number' || e.fx.speeds[t] ? !0 : typeof t !== 'string' || e.effects.effect[t] ? e.isFunction(t) ? !0 : typeof t !== 'object' || t.effect ? !1 : !0 : !0; }e.extend(e.effects, {
      ...
      removeWrapper(t) { const i = document.activeElement; return t.parent().is('.ui-effects-wrapper') && (t.parent().replaceWith(t), (t[0] === i || e.contains(t[0], i)) && e(i).focus()), t; },
    ...
    _create() {
      let t; const i = this; this.element.addClass('ui-selectable'), this.dragged = !1, this.refresh = function () {
    ...
    _processTabs() {
      const t = this; const i = this.tabs; const s = this.anchors; const n = this.panels;
      this.tablist = this._getList().addClass('ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all').attr('role', 'tablist').delegate('> li', `mousedown${this.eventNamespace}`, function (t) { e(this).is('.ui-state-disabled') && t.preventDefault(); })
