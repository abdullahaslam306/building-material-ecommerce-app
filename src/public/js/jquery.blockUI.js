...
/* jshint eqeqeq:false curly:false latedef:false */
  ...
  function setup($) {
    ...
    const ie6 = /MSIE 6.0/.test(navigator.userAgent) && !/MSIE 8.0/.test(navigator.userAgent);
    const mode = document.documentMode || 0;
    const setExpr = $.isFunction(document.createElement('div').style.setExpression);
    ...
    $.growlUI = function (title, message, timeout, onClose) {
      const $m = $('<div class="growlUI"></div>');
      if (title) $m.append(`<h1>${title}</h1>`);
      if (message) $m.append(`<h2>${message}</h2>`);
      if (timeout === undefined) timeout = 3000;

      // Added by konapun: Set timeout to 30 seconds if this growl is moused over, like normal toast notifications
      const callBlock = function (opts) {
        opts = opts || {};
      ...
      callBlock();
      const nonmousedOpacity = $m.css('opacity');
      $m.mouseover(() => {
        callBlock({
          fadeIn: 0,
          timeout: 30000,
    ...
    function install(el, opts) {
      ...
      let msg = (opts && opts.message !== undefined ? opts.message : undefined);
      ...
      let lyr1; let lyr2; let lyr3; let
        s;
      ...
      const layers = [lyr1, lyr2, lyr3]; const
        $par = full ? $('body') : $(el);
      ...
      const expr = setExpr && (!$.support.boxModel || $('object,embed', full ? null : el).length > 0);
      if (ie6 || expr) {
        ...
        if ((ie6 || !$.support.boxModel) && !full) {
          const t = sz(el, 'borderTopWidth'); const
        ...
        $.each(layers, (i, o) => {
          ...
          if (i < 2) {
            ...
            const top = (opts.css && opts.css.top) ? parseInt(opts.css.top, 10) : 0;
            const expression = `((document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop) + ${top}) + "px"`;
      ...
      if (opts.fadeIn) {
        const cb = opts.onBlock ? opts.onBlock : noOp;
    ...
    function remove(el, opts) {
      ...
      const full = (el == window);
      const $el = $(el);
      const data = $el.data('blockUI.history');
      const to = $el.data('blockUI.timeout');
      if (to) {
        clearTimeout(to);
    ...
    function reset(els, data, opts, el) {
      ...
      const body = $(document.body); const w = body.width(); const
    ...
    function center(el, x, y) {
      const p = el.parentNode; const
        s = el.style;
      const l = ((p.offsetWidth - el.offsetWidth) / 2) - sz(p, 'borderLeftWidth');
      const t = ((p.offsetHeight - el.offsetHeight) / 2) - sz(p, 'borderTopWidth');
